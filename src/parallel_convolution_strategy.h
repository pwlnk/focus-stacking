...
class ParallelConvolutionStrategy : public ConvolutionStrategy<TInput, TOutput> {
    private:
        void threadConvolutionChunk2D(int start_row, int end_row, Matrix<TInput>& image,
...
void ParallelConvolutionStrategy<TInput, TOutput>::convolution2D(Matrix<TInput>& image, Matrix<TOutput>& filtered_image, FilterKernel& kernel) {
    ...
    for (int thread_idx = 0; thread_idx < threads_number; thread_idx++) {
        ...
        threads.push_back(new std::thread([this, start_row, end_row, &image, &filtered_image, &kernel]
                                          { this->threadConvolutionChunk2D(start_row, end_row, image, filtered_image, kernel); }));
...
template <typename TInput, typename TOutput>
void ParallelConvolutionStrategy<TInput, TOutput>::threadConvolutionChunk2D(int start_row, int end_row, Matrix<TInput>& image,
                                                                            Matrix<TOutput>& filtered_image, FilterKernel& kernel) {
...
template <typename TInput, typename TOutput>
float ParallelConvolutionStrategy<TInput, TOutput>::convolution2DStep(int col, int row, int channel, Matrix<TInput>& image,
                                                                      Matrix<TOutput>& filtered_image, FilterKernel& kernel) {
...
template <typename TInput, typename TOutput>
float ParallelConvolutionStrategy<TInput, TOutput>::convolution1DStep(int col, int row, int channel, int kernel_direction, Matrix<TInput>& image,
                                                                    Matrix<TOutput>& filtered_image, Matrix<TOutput>& after_first_pass_image, FilterKernel& kernel) {
